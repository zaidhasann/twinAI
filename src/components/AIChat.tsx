...
export function AIChat({ isOpen, onClose }: AIChatProps) {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 bg-[#3A2A22] border border-white/[0.12] rounded-2xl shadow-2xl flex flex-col transition-all duration-300 overflow-hidden ${widthClass} ${heightClass}`}
    >
      {/* Chat Header */}
      <div className="p-4 border-b border-white/[0.08] bg-[#2A1E18]/90 backdrop-blur-md flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-[#C56A4A] to-[#91A889] p-px flex items-center justify-center">
            <div className="w-full h-full bg-[#2A1E18] rounded-[6px] flex items-center justify-center">
          ...
          <div>
            <div className="text-sm font-semibold text-white flex items-center gap-2">
              Your Career Twin
              <span className="w-1.5 h-1.5 rounded-full bg-[#91A889] animate-ping" />
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
        {messages.map(msg => (
          ...
          >
            <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center ${msg.role === 'assistant' ? 'bg-gradient-to-tr from-[#C56A4A] to-[#91A889]' : 'bg-white/[0.1]'}`}>
              {msg.role === 'assistant' ? (
            ...
            <div
              className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'user'
                  ? 'bg-[#C56A4A] text-white rounded-tr-sm'
        ...
        {isTyping && (
          <div className="flex gap-2.5">
            <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-[#C56A4A] to-[#91A889] flex items-center justify-center shrink-0">
              <Bot className="w-4 h-4 text-white" />
            </div>
            <div className="bg-white/[0.05] px-3.5 py-3 rounded-2xl rounded-tl-sm flex gap-1 items-center">
      ...
      <div className="p-3 border-t border-white/[0.08] flex items-center gap-2 shrink-0">
        <input
          ...
          className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-[#C56A4A]/50 focus:bg-white/[0.07] transition-all"
        />
        <button
          onClick={send}
          disabled={!input.trim()}
          className="w-9 h-9 rounded-xl bg-[#C56A4A] hover:bg-[#A94F36] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
        >
