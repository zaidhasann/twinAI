...
export const RoadmapPage: React.FC = () => {
  ...
  const getDifficultyBadge = (diff: RoadmapTopic['difficulty']) => {
    switch (diff) {
      case 'Easy':
        return 'text-[#7ED6A5] bg-[#7ED6A5]/10 border-teal-500/20';
      case 'Medium':
        return 'text-[#2E9D6B] bg-[#2E9D6B]/10 border-indigo-500/20';
      case 'Hard':
        return 'text-[#F4B860] bg-[#F4B860]/10 border-amber-500/20';
    }
  };
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="bg-[#232A30] border border-white/[0.1] rounded-xl px-5 py-3.5 flex items-center gap-4 self-start sm:self-auto min-w-[220px]">
          <div>
            ...
            <div className="flex items-baseline gap-2 mt-0.5">
              ...
              <span className="text-xs text-[#7ED6A5] font-semibold">
      ...
      <div className="space-y-8 relative before:absolute before:inset-0 before:left-5 sm:before:left-8 before:w-0.5 before:bg-gradient-to-b before:from-[#7ED6A5] before:via-[#2E9D6B] before:to-gray-800 before:z-0">
        {phases.map((phase) => {
            ...
            <div key={phase.id} className="relative z-10 pl-12 sm:pl-20">
              <div
                className={`absolute left-2 sm:left-5 top-1.5 w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-all ${
                  isPhaseCompleted
                    ? 'bg-[#7ED6A5] border-[#7ED6A5] text-black shadow-lg shadow-teal-500/30'
                    : isPhaseInProgress
                    ? 'bg-[#2E9D6B] border-[#2E9D6B] text-white shadow-lg shadow-indigo-500/30 ring-4 ring-indigo-500/20'
                    : 'bg-[#171B1F] border-gray-700 text-gray-500'
                }`}
              >
                {isPhaseCompleted ? (
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                           isPhaseCompleted
                            ? 'text-[#7ED6A5] bg-[#7ED6A5]/10 border-teal-500/30'
                            : isPhaseInProgress
                            ? 'text-[#2E9D6B] bg-[#2E9D6B]/10 border-indigo-500/30'
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-xl border flex flex-col justify-between gap-3 transition-all ${
                          isCompleted
                            ? 'bg-[#171B1F]/40 border-white/[0.05] opacity-80'
                            : isInProgress
                            ? 'bg-[#171B1F] border-[#2E9D6B]/40 shadow-md shadow-indigo-950/30'
                            : 'bg-[#171B1F] border-white/[0.06] hover:border-white/[0.12]'
                        }`}
                      >
                          ...
                          <div className="flex items-start gap-2.5">
                            <button
                              ...
                              className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 cursor-pointer transition-colors ${
                                isCompleted
                                  ? 'bg-[#7ED6A5] border-[#7ED6A5] text-black'
                                 : isInProgress
                                 ? 'border-[#2E9D6B] text-[#2E9D6B]'
                                 : 'border-white/[0.2]'
                              }`}
                            >
                              {isCompleted ? (
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer ${
                              isCompleted
                                ...
                                : isInProgress
                                ? 'bg-[#2E9D6B] text-white hover:bg-[#237B53]'
