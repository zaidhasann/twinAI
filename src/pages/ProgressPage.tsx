...
export const ProgressPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-1 bg-[#3A2A22] border border-white/[0.08] p-1 rounded-xl self-start sm:self-auto">
          {(['7 Days', '30 Days', '3 Months', '1 Year'] as const).map((t) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors cursor-pointer ${timeframe === t ? 'bg-[#C56A4A] text-white' : 'text-gray-400 hover:text-white'}`}
            >
              {t}
            </button>
          ))}
      ...
      <div className="bg-[#3A2A22] border border-white/[0.08] rounded-2xl p-6 sm:p-7 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-1.5 text-gray-300">
              <span className="w-3 h-3 rounded-full bg-[#C56A4A]" />
            ...
            <AreaChart data={MOCK_PROGRESS_HISTORY.readiness}>
              <defs>
                <linearGradient id="readinessGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#C56A4A" stopOpacity={0.4} />
                  <stop offset="95%" stopColor="#C56A4A" stopOpacity={0.0} />
                </linearGradient>
              </defs>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#3A2A22] border border-white/[0.08] rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded bg-[#91A889]/10 text-[#91A889]">
              +14% vs Last Week
            </span>
          </div>
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={MOCK_PROGRESS_HISTORY.studyHours}>
                ...
                />
                <Bar dataKey="hours" fill="#91A889" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={MOCK_PROGRESS_HISTORY.problemsSolved}>
                ...
                />
                <Bar dataKey="count" fill="#C56A4A" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
